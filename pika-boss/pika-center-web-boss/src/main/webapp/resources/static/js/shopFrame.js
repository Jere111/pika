		  ...
		  //获取对象
		   ...
		   //当窗口发生改变是触发
			 $(window).resize(function(){
			   ...
			   style_height.height($(window).height()-20);//获取栏目高度
				 //content.height($(window).height()-((options.header)+(options.bottom))).css({"width":$(window).width()-(options.menu),"margin-top":options.header,"margin-left":options.menu,"position":"relative"});
                 content.height($(window).height()-((options.header)+(options.bottom))).css({"margin-top":options.header,"margin-left":options.menu,"position":"relative"});
                 if($(showbtn).css('display') == "none"){
                     $(showbtn).click();
                 }else{
                     $(closeBtn).click();
                 }
		...
		oUl.stop().animate({'left':-num*100},100);
			...
			/*换肤*/
				...
				li.on('click',function(){
				  ...
				  $("#skin").attr("href","resources/static/skin/"+v+"/skin.css");
					});
